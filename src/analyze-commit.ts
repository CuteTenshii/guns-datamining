...
Be specific and cite the concrete tokens/strings/paths you saw. If most of the diff is minified/hashed churn with no discernible meaning, say so plainly rather than inventing significance. Keep it tight — skip filler.`;
  ...
  console.log(`Requesting analysis from ${DEEPSEEK_MODEL} (prompt ${prompt.length} chars)...`);
  ...
  if (!res.ok) {
    throw new Error(`DeepSeek ${res.status} ${res.statusText}: ${await res.text()}`);
  }
  ...
  console.log(
    `Model responded in ${elapsed}s (finish_reason: ${finishReason}, ` +
      `in: ${data.usage?.prompt_tokens ?? 0} out: ${data.usage?.completion_tokens ?? 0} tokens, ` +
      `${analysis.length} chars of analysis).`,
  );

  if (!analysis) {
    console.log(`Model returned no text (finish_reason: ${finishReason}). Exiting.`);
    return;
  }

  const body = `${MARKER}\n## 🔍 Commit analysis\n\n${analysis}\n\n<sub>Generated by DeepSeek (\`${DEEPSEEK_MODEL}\`) · [analyze-commit.ts](../blob/master/src/analyze-commit.ts)</sub>`;
